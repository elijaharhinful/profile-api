...
function buildFilter(query: Record<string, unknown>): Prisma.ProfileWhereInput {
  const str = (v: unknown): string | undefined => (v && typeof v === "string" ? v : Array.isArray(v) ? (v[0] as string) : undefined);
  ...
  if (min_gender_probability) filter.gender_probability = { gte: parseFloat(min_gender_probability) };
  if (min_country_probability) filter.country_probability = { gte: parseFloat(min_country_probability) };
...
export async function createProfile(req: Request, res: Response): Promise<void> {
  ...
  if (existing) {
    res.status(200).json({ status: "success", message: "Profile already exists", data: existing });
    return;
  }
  ...
  try {
    ...
    if (e.statusCode === 502 && e.api) {
      res.status(502).json({ status: "error", message: `${e.api} returned an invalid response` });
      return;
    }
  ...
  const profile = await prisma.profile.create({
    data: { id: uuidv7(), name: normalizedName, ...enriched, created_at: new Date() },
  });
...
export async function getAllProfiles(req: Request, res: Response): Promise<void> {
  ...
  const limit = Math.min(req.query.limit ? parseInt(req.query.limit as string, 10) : 10, 50);
  ...
  try {
    ...
    res.status(200).json({ status: "success", page, limit, total, total_pages, links, data });
  } catch {
    res.status(500).json({ status: "error", message: "Internal server error" });
  }
}
...
export async function searchProfiles(req: Request, res: Response): Promise<void> {
  ...
  if (!q || typeof q !== "string" || q.trim().length === 0) {
    res.status(400).json({ status: "error", message: "Missing query parameter 'q'" });
    return;
  }
  ...
  if (parsed.min_age !== undefined || parsed.max_age !== undefined) {
    prismaFilter.age = {};
    if (parsed.min_age !== undefined) (prismaFilter.age as Prisma.IntFilter).gte = parsed.min_age;
    if (parsed.max_age !== undefined) (prismaFilter.age as Prisma.IntFilter).lte = parsed.max_age;
  }
